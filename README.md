...
## 8) Typical Demo Flow (Recommended)

Use this order during presentation:
1. show input image
2. show enhanced result
3. show segmentation mask and overlay
4. show K-means cluster map and bright-cluster highlight
5. explain 3 analysis graphs
6. open web UI and run another image live
...
- If dependencies fail, ensure virtual environment is activated.
- If port 8000 is busy, edit `web_ui.py` server port to another value (for example 8001).
- If processing is slow, reduce `max_size` (for example 1200 or 1000).

---

## 10) Academic Note

This project combines classical DIP + unsupervised ML:
- DIP: CLAHE, thresholding, morphology, connected components, contours
- ML: K-means clustering over color + spatial features

It is suitable for demonstrating:
- image enhancement
- segmentation quality
- ML-based region grouping
- quantitative and visual analysis outputs
